...
class MemberChildIllness_model extends CI_Model
{
    ...
    {
        $this->db->where('id', $id);
        $this->db->update('tbl_member_child_illness', $IDInfo);
        
        return TRUE;
    }

    function getRoundwisechildIllness($household_master_id,$round_master_id)
    {
        $this->db->select('det.id,det.member_master_id, det.household_master_id,mm.member_code, mm.member_name,mm.birth_date,household_master_id_hh');
        $this->db->select('det.id,det.member_master_id, det.household_master_id,mm.member_code, mm.member_name,mm.birth_date,household_master_id_hh');
        $this->db->from('tbl_member_child_illness  det');
    ...
    function getChildIllnessDetailsByIdnHousehold($id,$household_master_id,$round_master_id)
    {
