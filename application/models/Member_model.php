...
class Member_model extends CI_Model
{
    ...
    {
        $this->db->select('a.id, a.birth_date, a.member_name, a.member_code, a.fk_marital_status, 
        a.fk_sex, a.fk_religion, a.fk_relation_with_hhh, a.fk_father_id, a.fk_mother_id, a.fk_spouse_id, 
        ...
        a.fk_education_id_last, a.fk_occupation_id_last, a.fk_member_relation_id_last, a.fk_mother_live_birth_order,
         ...
         a.updatedOn,hm.fk_extinct_type,hm.household_code,current_indenttification_id');
	...
	function getImmunizationListInfo($id,$tableName)
    {
        $this->db->select('a.id, a.birth_date, a.member_name, a.member_code, a.fk_marital_status, a.fk_sex, 
		a.fk_religion, a.fk_relation_with_hhh, a.fk_father_id, a.fk_mother_id, a.fk_spouse_id, 
		a.father_code, a.mother_code, a.spouse_code, a.household_master_id_hh, a.member_household_id_last,  
        ...
        $this->db->join('tbl_lookup_details en', 'en.id = mh.fk_entry_type', 'left');
        $this->db->join('tbl_lookup_details sex', 'sex.id = a.fk_sex', 'left');
        $this->db->where('(DATEDIFF(DAY, birth_date, GetDate()) / 365) <= 2');
        $this->db->where('a.id', $id);
        $query = $this->db->get();
	...
	function getMemberMasterPresentListByHouseholdIdsforImmunization($household_master_id)
    {
    ...
    function getAutocompleteMemberIn($search)
    {
       ...
       $this->db->join('tbl_member_household mh','mh.id= m.member_household_id_last','inner');
       $this->db->where("m.is_died",0);
	   $this->db->where("(mh.household_master_id = 0 or mh.fk_exit_type = 79)");
	   $this->db->where("(m.member_name like '%".$search."%' or m.member_code like '%".$search."%')");
	   $this->db->order_by("mh.household_master_id",'asc');
    ...
    function getMemberMasterPresentListByHouseholdIdnSex($household_master_id,$sexCode)
    {
        ...
        $this->db->join('tbl_member_household mh', 'mh.id = mm.member_household_id_last', 'inner');
        $this->db->where('mh.household_master_id', $household_master_id);
        $this->db->where('mm.fk_sex', $sexCode);
        $this->db->where('mh.round_master_id_exit_round', 0);
        $this->db->order_by('mm.member_code', 'asc');
        $query = $this->db->get()->result();
    ...
    function getMemberMasterPresentListByHouseholdIdnAll($household_master_id)
    {
    ...
    // function getMemberMasterPresentListByHouseholdIdnSex($household_master_id)
    //  {
    //         $this->db->select('mm.id,member_code,member_name');
    //         $this->db->from('tbl_member_master mm');
    //         $this->db->join('household_master hm', 'hm.id = mm.household_master_id_hh', 'inner');
    //         $this->db->join('tbl_member_household mh', 'mh.id = mm.member_household_id_last', 'inner');
    //         $this->db->where('mh.household_master_id', $household_master_id);
    //         $this->db->where('mm.fk_sex', 25);
    //         $this->db->where('mh.round_master_id_exit_round', 0);
    //         $query = $this->db->get();
    //         $output = '<option value="">--- Select Father ---</option>';
    //         foreach($query->result() as $row)
    //         {
    //         $output .= '<option value="'.$row->id.'">'.$row->member_code .'-'.$row->member_name.'</option>';
    //         }
    //       return $output;
    //  }
    ...
    function getMemberMasterPresentListByHouseholdIdsnAge($household_master_id)
    {
        ...
        $this->db->where('mh.round_master_id_exit_round', 0);
        $this->db->where('(DATEDIFF(DAY, birth_date, GetDate()) / 365) >= 10');
        $this->db->order_by('mm.member_code', 'asc');
        $query = $this->db->get()->result();
    ...
    {
        $this->db->select('a.id, a.birth_date, a.member_name, a.member_code, a.fk_marital_status, a.fk_sex, a.fk_religion, a.fk_relation_with_hhh, 
        a.fk_father_id, a.fk_mother_id, a.fk_spouse_id, a.father_code, a.mother_code, a.spouse_code, a.household_master_id_hh, 
        a.member_household_id_last,  a.fk_education_id_last, a.fk_occupation_id_last, a.fk_member_relation_id_last, 
        a.fk_mother_live_birth_order, a.national_id, a.fk_birth_registration, a.birth_registration_date, 
        a.fk_why_not_birth_registration, a.fk_additionalChild, a.afterYear, a.transfer_complete, a.insertedBy, 
        a.insertedOn, a.updateBy, a.updatedOn,hm.fk_extinct_type,hm.household_code, 
        hm.fk_district_id, hm.fk_thana_id, hm.fk_slum_id,hh.change_date, hm.fk_slum_area_id,mh.fk_entry_type,mh.entry_date, 
        fk_religious_edu,fk_secular_edu,fk_education_type,year_of_education,fk_main_occupation,en.code as entryTypeCode, 
        en.name as entryTypeName,a.birth_time,a.birth_weight, a.fk_birth_weight_size, a.keep_follow_up,pregnancy_outcome_id,
        fk_pnc_chkup_child_id,pnc_chkup_child_times,fk_pnc_first_visit_id,pnc_first_visit_days,fk_pnc_second_visit_id,
        pnc_second_visit_days,fk_post_natal_child_visit,fk_child_second_visit_assist,fk_child_first_visit_assist');
    ...
    function getMemberMasterPresentMarriedListByHouseholdIds($household_master_id,$matiatlStatusCode)
    {
	 ...
	 function getMemberMasterPresentListByHouseholdIdsandMaritalStatusmarsep($household_master_id)
    {
        ...
        $this->db->where('mh.household_master_id', $household_master_id);
        $this->db->where('mh.round_master_id_exit_round', 0);
		$this->db->where_in('mm.fk_marital_status', array(41,42));
        $this->db->order_by('mm.member_code', 'asc');
        $query = $this->db->get()->result();
