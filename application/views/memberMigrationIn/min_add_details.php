...
$( document ).ready(function() {
	     ...
	     var selslumID = '<?php echo $slumIDn ?>';  
		if (selslumID > 0)
		{
			  ...
			  if(selslumID != '')
			  {
			   $.ajax({
				...
				success:function(data)
				{
				 ...
				 $('#slumAreaID').val('<?php echo $slumAreaIDn; ?>').trigger('change');
				}
			   });
		 ...
		 var seldivisionID = '<?php echo $divisionIDn ?>';   
		if (seldivisionID > 0)
		{
			  ...
			  if(divisionID != '')
			  {
			   $.ajax({
				...
				success:function(data)
				{
				 ...
				 $('#districtID').val('<?php echo $districtIDn; ?>').trigger('change');
                                     ...
                                     <legend class="scheduler-border">Movement Information</legend>
											...
											<div class="form-group">
												...
												<select class="form-control required" id="fk_movement_type" name="fk_movement_type" required>
													...
													if(!empty($memberexittyp))
													{
														...
														{
															?>
															<option <?php if($fk_exit_typen == $memberexittyp->id) { echo "selected=selected";} ?> value="<?php echo $memberexittyp->id ?>"><?php echo $memberexittyp->code. '-' .$memberexittyp->name ?></option>
															<?php
														}
													}
													?>
										...
										<div class="col-md-4">
											<div class="form-group">
												...
												<input type="text" class="form-control" data-provide="datepicker-inline" id="movement_date" value="<?php echo $movement_daten ?>"  name="movement_date" required>
											</div>
										</div>
											...
											<div class="form-group">
												...
												<select class="form-control required fk_internal_cause" id="fk_internal_cause" name="fk_internal_cause">
													...
													if(!empty($internal_movement_cause))
													{
														...
														{
															?>
															<option <?php if($fk_internal_causen == $internal_movement_cause->id) { echo "selected=selected";} ?> value="<?php echo $internal_movement_cause->id ?>"><?php echo $internal_movement_cause->code. '-' .$internal_movement_cause->name ?></option>
															<?php
														}
													}
													?>
												...
												<select class="form-control required slumID" id="slumID" name="slumID">
												   ...
												   <?php
													if(!empty($slumlist))
													{
														...
														{
															?>
															<option <?php if($slumIDn == $slum->id) { echo "selected=selected";} ?> value="<?php echo $slum->id ?>"><?php echo $slum->code. '-' .$slum->name ?></option>
															<?php
														}
													}
													?>
												...
												<select class="form-control fk_migration_cause" id="fk_migration_cause"  name="fk_migration_cause">
												   <option value="0">Select cause of in-migration</option>
													...
													if(!empty($outside_cause))
													{
														...
														{
															?>
															<option <?php if($fk_migration_causen == $outsidecausegroup->id) { echo "selected=selected";} ?> value="<?php echo $outsidecausegroup->id ?>"><?php echo $outsidecausegroup->code. '-' .$outsidecausegroup->name ?></option>
															<?php
														}
													}
													?>
												...
												<select class="form-control countryID" id="countryID"  name="countryID">
												   <option value="0">--- Select Country ---</option>
													...
													if(!empty($countrylist))
													{
														...
														{
															?>
															<option <?php if($countryIDn == $country->id) { echo "selected=selected";} ?> value="<?php echo $country->id ?>"><?php echo $country->code. '-' .$country->name ?></option>
															<?php
														}
													}
													?>
												...
												<select class="form-control" id="divisionID"  name="divisionID">
												   <option value="0">--- Select Division ---</option>
													...
													if(!empty($divisionlist))
													{
														...
														{
															?>
															<option <?php if($divisionIDn == $division->id) { echo "selected=selected";} ?> value="<?php echo $division->id ?>"><?php echo $division->code. '-' .$division->name ?></option>
															<?php
														}
													}
													?>
										...
										<div class="col-md-4 migration country">
										 ...
										 <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <textarea  class="form-control" name="remarks"><?php echo $remarksn ?></textarea>
          ...
          if(slumID != '')
          {
           $.ajax({
            ...
            success:function(data)
            {
             $('#slumAreaID').html('');
             $('#slumAreaID').html(data);
			 $('#slumAreaID').val('<?php echo $slumIDn; ?>').trigger('change');
          ...
          if(slumAreaID != '')
          {
           $.ajax({
            ...
            success:function(data)
            {
			 ...
			 $('#househodID').val('<?php echo $slumAreaIDn; ?>').trigger('change');
          ...
          if(districtID != '')
          {
           $.ajax({
            ...
            success:function(data)
            {
             $('#thanaID').html('');
             $('#thanaID').html(data);
			 $('#thanaID').val('<?php echo $thanaIDn; ?>').trigger('change');
            }
           });
          }
          else
