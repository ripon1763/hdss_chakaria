...
class Upazila extends BaseController
{
	...
	public $controller = "upazila";
	public $pageTitle = 'Upazila Management';
	public $pageShortName = 'Upazila';
    ...
    public function index()
    {
            ...
            $data['userRecords'] = $this->modelName->listingUpa($this->config->item('upazilaTable'));
    ...
    function addNew()
    {
			...
			$data['district'] = $this->modelName->getDistrictList($this->config->item('districtTable'));
    ...
    function addNewList()
    {
            ...
            $this->form_validation->set_rules('name','Upazila Name','trim|required|max_length[255]|xss_clean');
            $this->form_validation->set_rules('code','Upazila Code','trim|required|max_length[10]|xss_clean');
            $this->form_validation->set_rules('districtID','District Name','trim|required|numeric|xss_clean');
		   ...
		   if($this->form_validation->run() == FALSE)
            ...
            else
            {
				...
				$IdInfo = array('name'=>$name,'code'=>$code,'districtID'=>$districtID, 'active'=>$active, 'insertedBy'=>$this->vendorId, 'insertedOn'=>date('Y-m-d H:i:s'));
					
				$result = $this->modelName->addNewList($IdInfo,$this->config->item('upazilaTable'));
    ...
    function editOld($id = NULL)
    {
		    ...
		    $this->global['menu'] =  $this->menuModel->getMenu($this->role);
		    $data['district'] = $this->modelName->getDistrictList($this->config->item('districtTable'));
            $data['userInfo'] = $this->modelName->getListInfoUpa($id,$this->config->item('upazilaTable'));
    ...
    function editList()
    {
            ...
            else
            {
                ...
                $IDInfo = array();
                $IDInfo = array( 'name'=> $name,'code'=> $code,'districtID'=> $districtID,
										'active'=>$active,'updatedBy'=>$this->vendorId, 'updatedOn'=>date('Y-m-d H:i:s'));
                
                $result = $this->modelName->editList($IDInfo, $id,$this->config->item('upazilaTable'));
