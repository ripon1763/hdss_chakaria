...
class MemberMigrationIn extends BaseController
{
    ...
    public function addMigrationIn()
    {
			...
			echo $data['fk_internal_causen'] = $this->session->userdata('fk_internal_cause') ?  $this->session->userdata('fk_internal_cause') : 0;
			echo $data['slumIDn'] = $this->session->userdata('slumID') ?  $this->session->userdata('slumID') : 0;
			echo $data['slumAreaIDn'] = $this->session->userdata('slumAreaID') ?  $this->session->userdata('slumAreaID') : 0;
			echo $data['househodIDn'] = $this->session->userdata('househodID') ?  $this->session->userdata('househodID') : 0;
			echo $data['divisionIDn'] = $this->session->userdata('divisionID') ?  $this->session->userdata('divisionID') : 0;
			echo $data['countryIDn'] = $this->session->userdata('countryID') ?  $this->session->userdata('countryID') : 0;
			echo $data['districtIDn'] = $this->session->userdata('districtID') ?  $this->session->userdata('districtID') : 0;
			echo $data['thanaIDn'] = $this->session->userdata('thanaID') ?  $this->session->userdata('thanaID') : 0;
			echo $data['fk_migration_causen'] = $this->session->userdata('fk_migration_cause') ?  $this->session->userdata('fk_migration_cause') : 0;
			echo $data['fk_exit_typen'] = $this->session->userdata('fk_exit_type') ?  $this->session->userdata('fk_exit_type') : 0;
			echo $data['movement_daten'] = $this->session->userdata('movement_date') ?  $this->session->userdata('movement_date') : '';
			echo $data['remarksn'] = $this->session->userdata('remarks') ?  $this->session->userdata('remarks') : '';
            ...
            else
            {
               ...
               if ($this->getCurrentRound()[0]->active == 0)
               {
                ...
                if ($this->config->item('internalMovementIn') == $fk_movement_type) 
                {
                    ...
                    $whereHHoldID = array('id' =>$member_household_id_last);
					$fk_exit_type = $this->db->select('fk_exit_type')->from($this->config->item('memberHouseholdTable'))->where($whereHHoldID)->get()->row()->fk_exit_type;
					...
					array(                    
							...
							'slumID'=>$this->input->post('slumID',true),
							'slumAreaID'=>$this->input->post('slumAreaID',true),
							'fk_exit_type'=>$this->input->post('fk_movement_type',true),
							'movement_date'=>$this->input->post('movement_date',true),
							'remarks' => $this->input->post('remarks',true),
							'househodID' => $this->input->post('househodID',true),
							'districtID' => 0,
