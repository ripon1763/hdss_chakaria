...
class dns1dbarcode
{
    ...
    * @public
    */
   public function getBarcodeSVG($code, $type,$w=1, $h=30, $color='black')
   {
    ...
    * @public
    */
   public function getBarcodeSVGcode($code, $type,$w=1, $h=30, $color='black')
   {
                $this->setBarcode($code, $type);
    ...
    * @public
    */
   public function getBarcodeHTML($code, $type,$w=1, $h=30, $color='black')
   {
                $this->setBarcode($code, $type);
      ...
      foreach ($this->barcode_array['bcode'] as $k => $v) {
         ...
         if ($v['t']) {
            $y = round(($v['p'] * $h / $this->barcode_array['maxh']), 3);
            // draw a vertical bar
            $html .= '<div style=" -webkit-print-color-adjust: exact;background-color:'.$color.';width:'.$bw.'px;height:'.$bh.'px;position:absolute;left:'.$x.'px;top:'.$y.'px;">&nbsp;</div>'."\n";
         }
         $x += $bw;
      }
      $html .= '</div>'."\n";
    ...
    * @public
    */
   public function getBarcodePNG($code, $type,$w=1, $h=30, $color=array(0,0,0))
   {
                $this->setBarcode($code, $type);
    ...
    * @public
    */
   public function getBarcodePNGPath($code, $type,$w=1, $h=30, $color=array(0,0,0))
   {
                $this->setBarcode($code, $type);
      // calculate image size
      $width = ($this->barcode_array['maxw'] * $w);
